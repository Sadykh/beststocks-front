    ...
    <div>
        ...
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="active">Назначение курьера</li>
            </ol>
        </section>
                    ...
                    <div class="box">
                        ...
                        <b-form @submit="formSubmit">
                            ...
                            <div class="box-body">
                                ...
                                <b-card class="mb-2">
                                    <b-form-group label="Курьер" label-for="courier">
                                        <b-form-select id="company" v-model="form.courier"
                                                       :options="courierListOptions"
                                                       class="form-control">
                                            <template slot="first">
                                                <option :value="null" disabled>Выберите курьера</option>
                                            </template>
                                        </b-form-select>
                                    </b-form-group>
                                </b-card>
    ...
    export default {
        computed: {
            ...
            ...mapGetters({
                ...
                errors: 'order/errors',
                courierList: 'courier/items',
                item: 'order/item',
            }),
            courierListOptions() {
                return this.courierList.map((item) => {
                    return {
                        value: item.id,
                        text: item.username
                    }
        ...
        methods: {
            async load() {
                this.$store.dispatch('courier/fetchAllWithoutPagination')
                const orderLoaded = await this.$store.dispatch('order/fetchOne', this.$route.params.id);
                if (orderLoaded) {
                    this.form.courier = this.item.courier.id;
                }
            },
            async formSubmit(e) {
                e.preventDefault()
                ...
                const result = await this.$store.dispatch('order/setCourier', {id: orderId, data: payloadData})
                if (result) {
                    this.$router.push({name: 'OrderView', params: {id: this.item.number}})
                    toastr.success('Курьер назначен')
                }
            }
        }
    }
